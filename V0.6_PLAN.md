...
| Issue | Risk | Solution |
|-------|------|----------|
| RLS circular reference | Query inefficiency | Use `EXISTS` subquery pattern instead of self-reference |
...
| Owner can't see members if not in group_members | Owner locked out of management | Added owner check to group_members SELECT policy |
...
- `ALREADY_MEMBER` - User is already in group (includes owner re-adding self)
...
-- Group Members: Users can view members of groups they belong to OR own
-- Note: Owner check ensures creator can always manage even if membership row missing
CREATE POLICY "Members can view group members"
  ...
  USING (
    ...
    -- OR user is the owner of this group
    OR EXISTS (
